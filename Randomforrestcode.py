...
df = yf.download(
    tickers= ["LLY", "JNJ", "ABBV", "RHHBY", "AZN", "NVS", "NVO", "MRK", "AMGN", "PFE", "GILD", "SNY", "SNY", "BMY", "GSK", "BIIB", "OGN"],
    start="2020-01-01",
    end="2025-01-01",
    interval="1d",
...
def get(tickers, startdate, enddate):
    def data(ticker):
        full = pd.read_csv('./Master Thesis data/Coin data/New coins/With volume/' + ticker + '.CSV', parse_dates=[3], index_col='Date')
...
def prep_data(tickers, start, end, K):
    ...
    for coin in tickers:
        # Coin 1 . reversal t-1
        dly_vars.loc[dly_vars.Ticker == coin, 't-1'] = dly_vars.loc[dly_vars.Ticker == coin, 'Price'].pct_change()
        # Coin 2.1 . momentum J=3
        dly_vars.loc[dly_vars.Ticker == coin, 'J_3'] = (dly_vars.loc[dly_vars.Ticker == coin, 't-1'] + 1).shift(2).rolling(3 - 1).apply(np.prod) - 1
        # # Coin 2.2 . momentum J=7
        dly_vars.loc[dly_vars.Ticker == coin, 'J_7'] = (dly_vars.loc[dly_vars.Ticker == coin, 't-1'] + 1).shift(
            1).rolling(7 - 1).apply(np.prod) - 1
        # # Coin 2.3 . momentum J=15
        dly_vars.loc[dly_vars.Ticker == coin, 'J_15'] = (dly_vars.loc[dly_vars.Ticker == coin, 't-1'] + 1).shift(
            1).rolling(15 - 1).apply(np.prod) - 1
        # # Coin 2.4 . momentum J=30
        dly_vars.loc[dly_vars.Ticker == coin, 'J_30'] = (dly_vars.loc[dly_vars.Ticker == coin, 't-1'] + 1).shift(
            1).rolling(30 - 1).apply(np.prod) - 1
        # Coin 3 . volatility t-7:t-1
        dly_vars.loc[dly_vars.Ticker == coin, 'J_std_3'] = (dly_vars.loc[dly_vars.Ticker == coin, 't-1'] + 1).shift(
             1).rolling(3 - 1).std()
        dly_vars.loc[dly_vars.Ticker == coin, 'J_std_7'] = (dly_vars.loc[dly_vars.Ticker == coin, 't-1'] + 1).shift(
             1).rolling(7 - 1).std()
        dly_vars.loc[dly_vars.Ticker == coin, 'J_std_15'] = (dly_vars.loc[dly_vars.Ticker == coin, 't-1'] + 1).shift(
             1).rolling(15 - 1).std()
        dly_vars.loc[dly_vars.Ticker == coin, 'J_std_30'] = (dly_vars.loc[dly_vars.Ticker == coin, 't-1'] + 1).shift(
             1).rolling(30 - 1).std()
        dly_vars.loc[dly_vars.Ticker == coin, 't-1'] = dly_vars.loc[dly_vars.Ticker == coin, 't-1'].shift(1)

        #mkt vars
        dly_vars.loc[dly_vars.Ticker == coin, 'mkt_t-1'] = mkt_ret.values
        dly_vars.loc[dly_vars.Ticker == coin, 'mkt_J_ret_3'] = mkt_cum_3.values
        dly_vars.loc[dly_vars.Ticker == coin, 'mkt_J_ret_7'] = mkt_cum_7.values
        dly_vars.loc[dly_vars.Ticker == coin, 'mkt_J_ret_15'] = mkt_cum_15.values
        dly_vars.loc[dly_vars.Ticker == coin, 'mkt_J_ret_30'] = mkt_cum_30.values
        dly_vars.loc[dly_vars.Ticker == coin, 'mkt_J_std_3'] = mkt_std_3.values
        dly_vars.loc[dly_vars.Ticker == coin, 'mkt_J_std_7'] = mkt_std_7.values
        dly_vars.loc[dly_vars.Ticker == coin, 'mkt_J_std_15'] = mkt_std_15.values
        dly_vars.loc[dly_vars.Ticker == coin, 'mkt_J_std_30'] = mkt_std_30.values
    # target returns
    for coin in tickers:
        # target
        dly_vars.loc[dly_vars.Ticker == coin, 'target'] = \
            dly_vars.loc[dly_vars.Ticker == coin, 'Price'].shift(-K) / dly_vars.loc[dly_vars.Ticker == coin, 'Price'] - 1
...
# # print(f"Accuracy on training set (kNN): {(kNN.score(X_train_scaled, y_train) * 100):.3f} %")
# # print(f"Accuracy on test set (kNN): {(kNN.score(X_test_scaled, y_test) * 100):.3f} %")
...
# print(f"Accuracy on training set (DTR): {(tree.score(X_train_nonscaled, y_train) * 100):.3f} %")
# print(f"Accuracy on test set (DTR): {(tree.score(X_test_nonscaled, y_test) * 100):.3f} %")
